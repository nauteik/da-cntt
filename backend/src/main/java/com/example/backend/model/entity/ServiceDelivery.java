...
@NoArgsConstructor
@ToString(exclude = { "scheduleEvent", "authorization", "dailyNotes", "checkEvents" })
public class ServiceDelivery extends BaseEntity {
  ...
  /**
   * Reason for unscheduled visit (e.g., "Original staff unavailable", "Emergency
   * replacement")
   */
  ...
  public void addCheckOutEvent(CheckEvent checkEvent) {
    ...
    if (checkIn != null && checkEvent.getOccurredAt() != null) {
      long seconds = java.time.Duration.between(
          ...
          checkEvent.getOccurredAt()).getSeconds();
