...
public class UserDataLoader {
  ...
  private void createUsersForRole(List<Office> offices, String roleCode, String[][] userData) {
    ...
    log.info("Created {} users for role: {}", userData.length, roleCode);
  }
  ...
  private void createStaffForUser(AppUser user, Office office, String[] data) {
    ...
    StaffContact staffContact = new StaffContact(staff, faker.options().option("Spouse", "Parent", "Sibling"),
        faker.name().fullName());
  ...
  private Office assignUserToOffices(AppUser user, List<Office> offices, String roleCode, int userIndex) {
    ...
    if ("ADMIN".equals(roleCode)) {
      ...
      // Other roles are assigned to a specific office based on index (cycling through
      // offices)
      if (!offices.isEmpty()) {
