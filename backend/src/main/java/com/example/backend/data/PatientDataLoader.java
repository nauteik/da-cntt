...
public class PatientDataLoader {
  ...
  public void loadData() {
    ...
    if (patientCount == 0 && officeCount > 0) {
      ...
      java.util.Map<java.util.UUID, java.util.List<PatientService>> patientIdToServices = patientServices.stream()
          .collect(java.util.stream.Collectors.groupingBy(ps -> ps.getPatient().getId()));
  ...
  private void loadPatientAddressData(Faker faker, List<Patient> patients, List<Address> addresses) {
    ...
    for (int i = 0; i < patients.size(); i++) {
      ...
      // This is required because check-in service validates
      // PatientAddress.latitude/longitude
    ...
    // Bulk insert patient addresses using JDBC batch processing for maximum
    // performance
    log.info("Bulk inserting {} patient-address links with GPS coordinates to database using JDBC batch processing...",
        patientAddresses.size());
  ...
  private void loadPatientContactData(Faker faker, List<Patient> patients, List<Address> addresses) {
    ...
    // Bulk insert patient contacts using JDBC batch processing for maximum
    // performance
    log.info("Created {} patient contacts", patientContacts.size());
  ...
  private List<Program> loadProgramData() {
    String[][] programs = {
        ...
        { "OLTL", "Office of Long-Term Living",
            "Provides long-term care services for seniors and adults with disabilities." },
    ...
    log.info("Created {} programs", savedPrograms.size());
  ...
  private List<PatientPayer> loadPatientPayerData(Faker faker, List<Patient> patients, List<Payer> payers) {
    ...
    log.info("Created {} patient payers", patientPayers.size());
  ...
  private void loadPatientProgramData(Faker faker, List<Patient> patients, List<Program> programs, List<Payer> payers) {
    ...
    for (Patient patient : patients) {
      ...
      patientProgram.setEnrollmentDate(
          faker.date().past(365, TimeUnit.DAYS).toInstant().atZone(ZoneId.systemDefault()).toLocalDate());
      patientProgram.setStatusEffectiveDate(
          faker.date().past(30, TimeUnit.DAYS).toInstant().atZone(ZoneId.systemDefault()).toLocalDate());
      patientProgram
          .setSocDate(faker.date().past(30, TimeUnit.DAYS).toInstant().atZone(ZoneId.systemDefault()).toLocalDate());
      patientProgram
          .setEocDate(faker.date().future(335, TimeUnit.DAYS).toInstant().atZone(ZoneId.systemDefault()).toLocalDate());
    ...
    // Bulk insert patient programs using JDBC batch processing for maximum
    // performance
    log.info("Created {} patient programs", patientPrograms.size());
    bulkInsertService.bulkInsertPatientPrograms(patientPrograms);
  }

  private List<PatientService> loadPatientServiceData(Faker faker, List<Patient> patients,
    ...
    for (Patient patient : patients) {
      ...
      ps.setStartDate(faker.date().past(120, java.util.concurrent.TimeUnit.DAYS).toInstant()
          .atZone(ZoneId.systemDefault()).toLocalDate());
      ps.setEndDate(faker.random().nextBoolean() ? null
          : faker.date().future(240, java.util.concurrent.TimeUnit.DAYS).toInstant().atZone(ZoneId.systemDefault())
              .toLocalDate());
      mappings.add(ps);
    }

    // Bulk insert patient services using JDBC batch processing for maximum
    // performance
    log.info("Created {} patient services", mappings.size());
    bulkInsertService.bulkInsertPatientServices(mappings);

    return mappings;
  }

  private void loadAuthorizationData(Faker faker, List<PatientPayer> patientPayers,
    ...
    for (PatientPayer patientPayer : patientPayers) {
      ...
      java.util.List<PatientService> services = patientIdToServices.getOrDefault(patientPayer.getPatient().getId(),
          java.util.Collections.emptyList());
      if (!services.isEmpty()) {
        auth.setPatientService(services.get(0));
      } else {
        // Skip creating authorization if no patient_service mapping exists to satisfy
        // NOT NULL constraint
        continue;
      }
    ...
    // Bulk insert authorizations using JDBC batch processing for maximum
    // performance
    log.info("Created {} authorizations", authorizations.size());
  ...
  private List<ISP> loadIspData(Faker faker, List<Patient> patients) {
    ...
    log.info("Created {} ISPs", isps.size());
  ...
  private List<ISPGoal> loadIspGoalData(Faker faker, List<ISP> isps) {
    ...
    log.info("Created {} ISP goals", ispGoals.size());
