...
public class CoreDataLoader {
  ...
  private void loadModules() {
    ...
    log.info("Created {} modules", modules.length);
  }
  ...
  private void loadRoles() {
    ...
    log.info("Created {} roles", roles.length);
  }
  ...
  private void loadPermissions() {
    ...
    for (String[] permData : permissions) {
      ...
      if (permissionRepository.findByResourceAndActionAndScope(resource, action, PermissionScope.valueOf(scope))
          .isEmpty()) {
    ...
    if (!permissionList.isEmpty()) {
      log.info("Creating {} permissions", permissionList.size());
  ...
  private void loadServiceTypes() {
    Object[][] serviceTypes = {
        ...
        { "HOME_COMM", "Home & Community Habilitation", CareSetting.NON_RESIDENTIAL,
            "Assists individuals in acquiring, maintaining, and improving self-help, domestic, and social skills.",
            "15min", true },
        { "COMPANION", "Companion Services", CareSetting.NON_RESIDENTIAL,
            "Provides daily companionship, assistance with travel, and social interaction.", "15min", true },
        { "EMP_SUPPORT", "Employment / Supported Employment", CareSetting.NON_RESIDENTIAL,
            "Supports individuals in finding, training for, and maintaining employment in an integrated setting.",
            "hour", true },
        { "THERAPY", "Therapy Services", CareSetting.NON_RESIDENTIAL,
            "Includes physical, speech, behavioral therapy, and psychological counseling.", "15min", true },
        { "BEHAVIOR", "Behavior Support", CareSetting.NON_RESIDENTIAL,
            "Behavioral assessment, intervention planning, and support to reduce challenging behaviors.", "15min",
            true },
        { "TRANSPORT", "Transportation", CareSetting.NON_RESIDENTIAL,
            "Assists with travel to programs, work, medical appointments, and social activities.", "trip", true },
        { "RESPITE_DAY", "Respite Services (Day/Night)", CareSetting.NON_RESIDENTIAL,
            "Provides short-term relief for primary caregivers, either in-home or during the day.", "hour", true },
        { "ASSISTIVE", "Assistive Technology / Environmental Modifications", CareSetting.NON_RESIDENTIAL,
            "Provides devices, home modifications, and tools to support mobility, communication, and safety.", "item",
            true },

        // Residential (SRS 3.2)
        { "GROUP_HOME", "Group Homes / Community Living", CareSetting.RESIDENTIAL,
            "Group homes or apartments with 24/7 staff support to ensure safety and daily living.", "day", true },
        { "LIFE_SHARING", "Life Sharing / Family Living", CareSetting.RESIDENTIAL,
            "Individuals live with a host family or their own family with continuous support.", "day", true },
        { "SUPPORTED_LIVING", "Supported / Independent Living with Supports", CareSetting.RESIDENTIAL,
            "Independent living with needs-based support for housekeeping, medical care, and medication management.",
            "day", true },
        { "ICF", "Intermediate Care Facilities (ICF/IID)", CareSetting.RESIDENTIAL,
            "Facilities with higher supervision, providing intensive medical/behavioral support for complex needs.",
            "day", true }
    };
    ...
    log.info("Created {} service types", serviceTypes.length);
  }
}
